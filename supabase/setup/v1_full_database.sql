...
-- House Care v1.0.1 — workspace management
alter table public.workspaces add column if not exists archived_at timestamptz;
create index if not exists workspaces_owner_archived_idx on public.workspaces(owner_user_id, archived_at);
...
begin
  ...
  update public.workspaces set archived_at = now() where id = target_workspace_id;
  update public.notification_jobs set status = 'cancelled', locked_at = null
   where workspace_id = target_workspace_id and status in ('pending','processing','failed');
end;
$$;
...
begin
  ...
  update public.workspaces set archived_at = null where id = target_workspace_id;
  for task_row in select id from public.task_occurrences where workspace_id = target_workspace_id and state = 'scheduled' loop
...
begin
  ...
  if not exists (select 1 from public.workspaces where id = target_workspace_id and archived_at is not null) then
...
-- BEGIN: supabase/migrations/20260829213000_v1_1_2_task_state_integrity.sql
...
-- END: supabase/migrations/20260829213000_v1_1_2_task_state_integrity.sql

-- BEGIN: supabase/migrations/20260831113000_v1_2_phase1_cleanliness_engine.sql
...
-- END: supabase/migrations/20260831113000_v1_2_phase1_cleanliness_engine.sql

-- BEGIN: supabase/migrations/20260831123000_v1_2_phase2_overview_undo.sql
...
-- END: supabase/migrations/20260831123000_v1_2_phase2_overview_undo.sql

-- v1.2.0 state consistency corrective definitions (see migration 20260831173000).
