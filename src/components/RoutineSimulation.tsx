...
export function RoutineSimulation({ data, routine }: { data: WorkspaceData; routine: Routine }) {
  ...
  return <div className="simulation-box"><div className="section-header"><div><strong>{t('simulation30')}</strong><small>{t('simulationHint')}</small></div><span className="count-pill">{rows.length}</span></div>
    {!rows.length ? <p className="muted compact-text">{t('noSimulationRows')}</p> : <div className="simulation-list">{rows.map((row, index) => <div className="simulation-row" key={`${row.dueAt}-${index}`}><div><strong>{formatTaskDateTime(row.dueAt, locale, data.workspace.timezone)}</strong><small>{row.targetNames.join(', ') || t('noMatchedTargets')}</small></div><div><span>{row.assigneeName ?? t('anyone')}</span>{row.conflicts.length > 0 && <small className="danger-text">{row.conflicts.join(' · ')}</small>}</div></div>)}</div>}
  </div>
}
