...
export function OverviewPage() {
  const { data, currentMember, completeTask, completeTaskTarget, skipTask, postponeTask, reassignTask, undoTaskAction, setSupplyStatus } = useData()
  ...
  return <div className="stack page-stack overview-page">
    ...
    <section className="overview-section critical-section">
      ...
      {groups.criticalItems.length === 0 ? <div className="quiet-state">{t('noCriticalItems')}</div> : <div className="critical-grid">{groups.criticalItems.map((item) => <article className="critical-card" key={item.itemId}>
        ...
        <div className="cleanliness-track" role="meter" aria-valuemin={0} aria-valuemax={100} aria-valuenow={Math.round(item.score)}><span style={{ width: `${item.score}%` }} /></div>
  ...
  function TaskSheet({ task, events, onClose }: { task: TaskOccurrence; events: TaskEvent[]; onClose: () => void }) {
    ...
    const canRestoreToday = Boolean(latestReversible && localDateInZone(timezone, new Date(latestReversible.at)) === today)
    return <Sheet title={task.routineNameSnapshot} onClose={onClose}><div className="stack">
      ...
      {task.state === 'scheduled' && <button className="button primary" onClick={() => void completeTask(task.id).then((eventId) => { void rememberUndo(task.id, eventId, `${task.actionNameSnapshot} ${t('completed').toLowerCase()}`); onClose() })}>{t('done')}</button>}
      {canRestoreToday && <button className="button secondary" onClick={() => { void undoTaskAction(task.id, latestReversible!.id).then(onClose) }}>{t('restoreToDoToday')}</button>}
      <section className="action-section"><div className="section-header"><h3>{t('taskTargets')}</h3><small>{task.targets.filter((target) => target.completedAt).length}/{task.targets.length}</small></div><div className="target-progress-list">{task.targets.map((target) => <div className={target.completedAt ? 'target-progress-row done' : 'target-progress-row'} key={target.entityId}><div><strong>{target.entityName}</strong><small>{target.entityTypeName}</small></div>{target.completedAt ? <span className="status">{t('completed')}</span> : task.state === 'scheduled' ? <button className="button secondary small" onClick={() => void completeTaskTarget(task.id, target.entityId)}>{t('markDone')}</button> : null}</div>)}</div></section>
      {task.supplies.length > 0 && <section className="action-section"><h3>{t('reportStock')}</h3><div className="task-supplies">{task.supplies.map((snapshot) => { const supply = data!.supplies.find((item) => item.id === snapshot.supplyId); if (!supply || supply.archivedAt) return null; return <div className="task-supply-row" key={snapshot.supplyId}><div><strong>{snapshot.supplyName}</strong><small>{statusLabel(supply.status)}</small></div><div className="mini-stock-grid">{stockStatuses.map((status) => <button key={status} aria-label={statusLabel(status)} className={supply.status === status ? 'selected' : ''} onClick={() => void setSupplyStatus(supply.id, status, task.id)}><span className={`stock-dot stock-${status}`} /></button>)}</div></div> })}</div></section>}
      {task.state === 'scheduled' && <><section className="action-section"><h3>{t('reschedule')}</h3><div className="inline-form"><input type="datetime-local" value={postponeValue} onChange={(event) => setPostponeValue(event.target.value)} /><button className="button secondary" onClick={() => { const value = localInputToUtc(postponeValue, timezone); void postponeTask(task.id, value).then((eventId) => { void rememberUndo(task.id, eventId, `${task.actionNameSnapshot} ${t('rescheduled').toLowerCase()}`); onClose() }) }}>{t('reschedule')}</button></div></section><section className="action-section"><h3>{t('reassign')}</h3><div className="inline-form"><select value={assignee} onChange={(event) => setAssignee(event.target.value)}><option value="">{t('anyone')}</option>{data!.members.filter((member) => member.status === 'active').map((member) => <option value={member.id} key={member.id}>{member.displayName}</option>)}</select><button className="button secondary" onClick={() => void reassignTask(task.id, assignee || undefined).then((eventId) => { void rememberUndo(task.id, eventId, `${task.actionNameSnapshot} ${t('reassigned').toLowerCase()}`); onClose() })}>{t('reassign')}</button></div></section><button className="button danger-outline" onClick={() => void skipTask(task.id).then((eventId) => { void rememberUndo(task.id, eventId, `${task.actionNameSnapshot} ${t('skipped').toLowerCase()}`); onClose() })}>{t('skip')}</button></>}
